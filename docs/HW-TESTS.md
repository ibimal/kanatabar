...
| 19  | Daemon-context TCC read **and request**: does a **root LaunchDaemon** report its own grant accurately via `IOHIDCheckAccess` / `AXIsProcessTrusted`, and does `IOHIDRequestAccess` / `AXIsProcessTrustedWithOptions` register kanatad's entry from the system context ("Set it up for me")? | 11  | `ffi/tcc.rs`; `doctor::{input_monitoring_check,accessibility_check}`; `control` RequestPermission | ✅ **READ CONFIRMED 2026-07-17** (macOS 26.5, installed root LaunchDaemon via `kanatactl install --daemon-only`). Ungranted (stale grant after a binary swap): `input monitoring: denied` (IOHIDCheckAccess returned **Denied**, not Unknown — even a stale post-update entry is caught) and `accessibility` not-trusted. After **remove + re-add** in both panes **and a daemon restart** (`launchctl kickstart -k system/…daemon`): both read `granted (verified via …)`, supervisor `Running`. So the daemon-context read is accurate **both** directions → code flipped (2026-07-17): not-granted (Denied *and* Unknown / not-trusted) now **fails** the check. ⚠️ **Key finding: the daemon's TCC verdict is launch-cached in BOTH directions** — grant-while-running kept reading Denied until `kickstart` (2026-07-17), and **revoke-while-running kept reading Granted 60+ s after the toggle** (re-test 2026-07-18, stable pid, plain Accessibility toggle — no binary-swap confound). GUI-session apps (Thaw) get live AX updates from the user tccd and a System-Settings-driven "Quit & Reopen" for Input Monitoring; a system-domain LaunchDaemon gets neither. Consequences: (1) the **request** ("Set it up for me") flow must restart kanatad after the user toggles the pane — **still to wire (Part-2 follow-up)**; (2) a revoked grant shows a **false green** until restart — the behavioral `Degraded` backstop (fresh kanata hits the real grant) stays essential. ✅ **Fresh-child probe CONFIRMED on HW 2026-07-18** (hidden `kanatad tcc-status` + `doctor::probe_tcc_status`, in-process fallback; the check detail names its source): after deploy + re-grant + kickstart, doctor read both green via "fresh probe"; toggling Accessibility **off** flipped the check ❌ **within seconds**, and toggling back **on** flipped it ✅ within seconds — **same daemon pid throughout, zero restarts**. So TCC evaluates a child of the responsible process afresh, and doctor/wizard polls now get **live** grant status (Thaw-style poll-until-green) despite the parent's launch-cached verdict. Thaw comparison re-verified in source 2026-07-18: Thaw uses NO Input Monitoring (Accessibility + Screen Recording only) and no restart logic — its live green is purely the GUI-session `AXIsProcessTrustedWithOptions(nil)` poll (AXSwift 0.3.2). ✅ **Input Monitoring probe + #19a CONFIRMED 2026-07-18** (same session, daemon pid stable throughout): IM toggle-off read ❌ `denied (fresh probe)` within seconds — the live fresh-child read holds for `IOHIDCheckAccess`, not just AX. **Revocation killed NEITHER process** (kanatad and the running kanata both survived; kanata's established opens kept working → doctor-red + `Running` is the truthful in-between state). Then `kanatactl restart` under revoked IM → fresh kanata denied → died with the TCC message → `Degraded{InputMonitoringDenied}` (deny half); re-grant → doctor ✅ live → plain `kanatactl restart` → `Running`, new kanata pid, **zero daemon restarts across the whole cycle** (grant half). **Auto-recovery IMPLEMENTED 2026-07-18** (`grant_timer` in the supervisor loop: armed only while `Degraded{InputMonitoringDenied}`, probes every 3 s, dispatches Restart when BOTH grants read present; the sole scoped exception to the zero-polling [HARD] rule — macOS has no TCC-change notification API). ✅ **Auto-recovery HW-CONFIRMED 2026-07-18** (the 12.4 deploy doubled as the test): install → fresh daemon's kanata denied (stale grants) → `Degraded{InputMonitoringDenied}` → user removed+re-added both grants and did nothing else → log shows `Input Monitoring + Accessibility now granted (fresh probe); restarting kanata` and `Degraded → Starting → Running` **65 ms later**; all 13 checks green, daemon pid unchanged, zero user commands after the toggles. The only remaining unverified TCC item is the "Set it up for me" pane **registration** from the system context (`IOHIDRequestAccess`/`AXIsProcessTrustedWithOptions`) — exercise it from the Setup Assistant's grant steps in Run 11. ✅ **Registration RESOLVED 2026-07-18 (Run 11 campaign): both request APIs are no-ops from the system context** — `AXIsProcessTrustedWithOptions(prompt)` executed 3× (logged, `granted=false`) with no pane entry appearing; `IOHIDRequestAccess` likewise registered nothing (socket-level test; user had to + manually). **"Set it up for me" removed** (button, `request:` page action, RequestPermission IPC, request FFI) — the honest grant-step UX is "Open System Settings" + the +/Cmd+Shift+G instruction. **Bonus TCC semantics finding (same session): for the root daemon, Input Monitoring is allow-unless-denied** — *removing* the pane entry entirely reads `granted` AND a fresh kanata genuinely works (verified by respawn); only an explicit deny record (entry toggled off) or a stale (pre-update) record blocks. Accessibility differs: no record = not-trusted. So "remove + re-add" guidance matters for AX and for deny-recovery, while a clean machine's kanatad needs no IM entry at all until something creates a record. |
